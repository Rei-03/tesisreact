...
export class CircuitosRepository {
    ...
    /**
     * Obtiene circuitos con consumo por fecha Y el último apagón de cada circuito
    ...
    async findWithConsumptionAndLastApagon(fecha: string, take = 20, skip = 0) {
        const result = await this.db.request()
            ...
            .query(`
                ...
                SELECT * FROM (
                    ...
                    LEFT JOIN ap_curvas cur ON c.idCircuitoP = cur.idCircuitoP 
                        AND DATEDIFF(DAY, cur.fecha, @fecha) = 0
                    LEFT JOIN ap_apagones ap ON c.idCircuitoP = ap.idCircuitoP
                    LEFT JOIN LastApagones la ON ap.idCircuitoP = la.idCircuitoP 
                        AND ap.idApagon = la.maxApagonId
                    WHERE ap.idCircuitoP IS NULL OR la.maxApagonId IS NOT NULL
                ) AS ResultWithRows
                WHERE RowNum > @skip AND RowNum <= (@skip + @take)
                ORDER BY CircuitoP
            `);
        ...
        return result.recordset.map(row => {
            ...
            // Estructura de respuesta con consumo y último apagón
