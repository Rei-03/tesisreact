    ...
    async findWithConsumption(take = 20, skip = 0) {
        const result = await this.db.request()
            ...
            SELECT * FROM (
                ...
                LEFT JOIN ap_curvas cur ON c.idCircuitoP = cur.idCircuitoP 
                    AND DATEDIFF(DAY, cur.fecha, GETDATE()) = 0
            ) AS ResultWithRows
            WHERE RowNum > @skip AND RowNum <= (@skip + @take)
            ORDER BY CircuitoP
        `);
        ...
        return result.recordset.map(row => {
            ...
            const {
                ...
                RowNum, ...datosCircuito
            } = row;

            return {
                ...datosCircuito,
                consumo: {
                    // El valor "mw" principal sigue siendo el de la hora actual
                    mw: row[`h${currentHour}`] || 0,
                    // Incluimos el histórico de las 24 horas en un objeto o array
                    historico: [
                        h0, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11,
                        h12, h13, h14, h15, h16, h17, h18, h19, h20, h21, h22, h23
                    ],
